    ...
    <div class="py-6 max-w-xl mx-auto bg-white shadow p-6 rounded">
        <form method="POST" action="{{ route('admin.folders.update', $folder) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name', $folder->name) }}" class="w-full mt-1 p-2 border rounded" required>
                @error('name') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium">Description</label>
                <textarea name="description" class="w-full mt-1 p-2 border rounded">{{ old('description', $folder->description) }}</textarea>
            </div>
